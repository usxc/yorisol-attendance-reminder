...
ヨリソルの出席確認を補助する Chrome 拡張機能です。授業開始から5分後にヨリソルを確認し、未出席・出席ボタン複数・出席ボタンなしなど、確認が必要な状態だけ通知します。
...
確認時に出席ボタンがまだ押されていない状態だと判定された場合は、ブラウザの通知で対象の時限・授業名を表示します。通知内のボタンをクリックすると、対象授業のモーダルを開き、対象日付の `targetKoma` 番目の授業ページ内にある出席ボタン位置まで移動します。

### 出席ボタンへの移動

![出席ボタン強調表示](docs/images/attendance-button-spotlight.png)

通知内のボタンから移動した場合、対象の出席ボタンまでスクロールし、ボタンを一時的に強調表示します。出席ボタンは自動で押しません。
...
通知確認は、授業開始から5分後に実行します。初回確認で出席ボタンが見つからない場合、対象日付の授業nodeがまだ見つからない場合、または確認用タブが途中で閉じられた場合は、授業終了前であれば5分後に1回だけ自動再確認します。出席ボタンが見つからない場合は、手動確認・自動確認のどちらでも通知します。自動確認で5分後の再確認を予約した場合はその旨を通知し、再確認後も出席ボタンが見つからない場合は手動確認を促す通知を出します。
...
8. 「出席済み」表示があるか確認
9. 「出席する」ボタンがあるか確認
10. 未出席・出席ボタン複数・出席ボタンなしなど、確認が必要な場合だけ通知
11. 出席ボタンなしなど再確認対象の結果では、必要に応じて5分後の自動再確認を1回だけ予約
12. 確認完了後、確認専用タブを閉じる
