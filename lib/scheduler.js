...
import { ALARM_PREFIX, DAILY_REBUILD_ALARM, RETRY_ALARM_MARKER } from "./constants.js";
...
export function buildRetryAlarmName(dateString, period, retryAttempt) {
  return `${ALARM_PREFIX}:${dateString}:${Number(period)}:${RETRY_ALARM_MARKER}:${Number(retryAttempt)}`;
}
...
export function parseAttendanceAlarmName(name) {
  ...
  if (parts.length === 5 && parts[0] === ALARM_PREFIX && parts[3] === RETRY_ALARM_MARKER) {
    const retryAttempt = Number(parts[4]);
...
export async function scheduleRetryAlarm(dateString, period, retryAttempt, when) {
  await createAlarm(buildRetryAlarmName(dateString, period, retryAttempt), { when });
}
