...
export async function notifyNoButton(target, result, options = {}) {
  const notificationId = NOTIFICATION_IDS.NO_BUTTON_PREFIX + ":" + target.slotId + ":" + Date.now();
  ...
  await chrome.notifications.create(notificationId, {
    type: "basic",
    iconUrl: "icons/icon-128.png",
    title: options.retryScheduled ? "出席ボタンを再確認します" : "出席ボタンが見つかりません",
    message: buildNoButtonMessage(target, options),
    priority: 2
  });
...
function buildNoButtonMessage(target, options = {}) {
  const prefix = String(target?.period || "") + "限 " + String(target?.displaySubject || "");
  if (options.retryScheduled) {
    return prefix + " ボタンなし。5分後に再確認します。";
  }
  if (options.afterRetry) {
    return prefix + " 再確認後もボタンなし。手動確認してください。";
  }
  return prefix + " ボタンなし。手動確認してください。";
}
